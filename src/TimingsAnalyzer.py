...
class TimingsAnalyzer:
    ...
    def __init__(self, sched_variant: 'SchedVariant', struct_variant: 'StructVariant', print_stalls=True):
    ...
    def analyse_steady_state(self, code_block: 'InstructionBlockDescription', final_timings: 'Timings' = None, timings_history: List['Timings'] = []):
        ...
        """
        print("\n-- FRONTEND: TIMINGS_ANALYZER --")
        if final_timings is None:
            assert len(timings_history) == len(code_block.instructions), f"Invalid timings!"
        ...
        for instr_idx, instr in enumerate(code_block.instructions):
            ...
            if self.print_stalls:
                ...
                if current_end_cycle > (expected_end_cycle + accumulated_stalls):
                    diff = current_end_cycle - expected_end_cycle - accumulated_stalls
        ...
        stall_cycles     = actual_end_cycle - expected_end_cycle
        num_instructions = len(code_block.instructions)

        if self.print_stalls:
            assert accumulated_stalls == stall_cycles, \
                   f"Stall cycles mismatch! ({accumulated_stalls} vs expected {stall_cycles})"

        cpi = (num_instructions + stall_cycles) / num_instructions
        return dotdict({ "cpi": cpi, "stall_cycles": stall_cycles, "num_instructions": num_instructions })
